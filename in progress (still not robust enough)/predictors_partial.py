...
#                                 pairInfor = [x/sum(pairInfor) for x in pairInfor] 
            ...
            #     Gtest[:, indCol] = 0
            #     Atest[:, indCol] = 0
            hard_prediction_indices, predicted_preorder= predictors._partialorders_reasoning_procedure(base_learner, vector, indices_vector, n_labels, Gtest, h, Atest, b, I, B)
            hard_prediction = [1 if x in hard_prediction_indices else 0 for x in range(n_labels)]
...
#                epist_00 += optX[indicesVector["%i_%i_%i"%(i,j,2)],0] 
#                aleat_11 += optX[indicesVector["%i_%i_%i"%(i,j,3)],0]             
        hard_prediction = [ind for ind in range(n_labels) if scores_d[ind] > 0 or scores_n[ind] == 0]
...
#                                 pairInfor = [x/sum(pairInfor) for x in pairInfor] 
            ...
            #     Gtest[:, indCol] = 0
            #     Atest[:, indCol] = 0
            hard_prediction_indices, predicted_preorder= predictors._preorders_reasoning_procedure(base_learner, vector, indices_vector, n_labels, Gtest, h, Atest, b, I, B)
            hard_prediction = [1 if x in hard_prediction_indices else 0 for x in range(n_labels)]
...
#                aleat_11 += optX[indicesVector["%i_%i_%i"%(i,j,3)],0]             
    ...
    def _hamming(self,
        ...
        for index in range(n_instances):
            ...
            hard_pred = [hard_pred[k] for k in range(n_labels) if k not in known_indices[index]]
            true_lab = [true_lab[k] for k in range(n_labels) if k not in known_indices[index]]
    ...
    def _f1(self,
        ...
        for index in range(n_instances):
            ...
            if max(hard_pred) == 0 and max(true_lab) == 0:
                ...
                f1 += (2*np.dot(hard_pred, true_lab))/(np.sum(hard_pred) + np.sum(true_lab))
...
#        return np.mean(jaccard_score(hard_predictions, true_label, average=None))
        ...
        for index in range(n_instances):
            ...
            if max(hard_pred) == 0 and max(true_lab) == 0:
                ...
                jaccard += (np.dot(hard_pred, true_lab))/(np.sum(hard_pred) + np.sum(true_lab) - np.dot(hard_pred, true_lab))
...
#    partial_prob = 0.1
    ...
    for dataFile in ['emotions.arff', 'scene.arff', 'CHD_49.arff', 'Yeast.arff','Water-quality.arff']: 
#    n_labels_set = [6, 6]
...
#                                          Y[train_index[index], k] = 1 # for additional test
                            ...
                            pairwise_2classifier, calibrated_2classifier = pairwise_classifiers._pairwise_2classifier(base_learner, n_labels, 
                                                                                    X[train_index], Y[train_index])
                            predicted_Y, predicted_ranks = predictors._CLR(n_labels, X[test_index], pairwise_2classifier, calibrated_2classifier)
                            hamming_loss_pairwise_2classifier = predictors._hamming(base_learner, predicted_Y, Y[test_index], known_indices)
                            ...
                            f1_pairwise_2classifier = predictors._f1(base_learner, predicted_Y, Y[test_index], known_indices)
                            average_f1_pairwise_2classifier.append(f1_pairwise_2classifier)
                            print(f1_pairwise_2classifier)
                            jaccard_pairwise_2classifier = predictors._jaccard(base_learner, predicted_Y, Y[test_index], known_indices)
                            average_jaccard_pairwise_2classifier.append(jaccard_pairwise_2classifier)
                            print(jaccard_pairwise_2classifier)   
                            subset0_1_pairwise_2classifier = predictors._subset0_1(base_learner, predicted_Y, Y[test_index], known_indices)
                            average_subset0_1_pairwise_2classifier.append(subset0_1_pairwise_2classifier)
                            print(subset0_1_pairwise_2classifier)   
                            recall_pairwise_2classifier = predictors._recall(base_learner, predicted_Y, Y[test_index], known_indices)
        ...
        #                    print(np.mean([np.mean(x) for x in hard_predictions]))
                            ...
                            pairwise_3classifier = pairwise_classifiers._pairwise_3classifier(base_learner, n_labels, 
                                                                                    X[train_index], Y[train_index])
                            predicted_Y, predicted_ranks = predictors._partialorders(n_labels, X[test_index], Y[test_index],
                                              known_indices, pairwise_3classifier, calibrated_2classifier)
                            hamming_loss_pairwise_3classifier = predictors._hamming(base_learner, predicted_Y, Y[test_index], known_indices)
                            ...
                            f1_pairwise_3classifier = predictors._f1(base_learner, predicted_Y, Y[test_index], known_indices)
                            average_f1_pairwise_3classifier.append(f1_pairwise_3classifier)
                            print(f1_pairwise_3classifier)
                            jaccard_pairwise_3classifier = predictors._jaccard(base_learner, predicted_Y, Y[test_index], known_indices)
                            average_jaccard_pairwise_3classifier.append(jaccard_pairwise_3classifier)
                            print(jaccard_pairwise_3classifier)
                            subset0_1_pairwise_3classifier = predictors._subset0_1(base_learner, predicted_Y, Y[test_index], known_indices)
                            average_subset0_1_pairwise_3classifier.append(subset0_1_pairwise_3classifier)
                            print(subset0_1_pairwise_3classifier)
                            recall_pairwise_3classifier = predictors._recall(base_learner, predicted_Y, Y[test_index], known_indices)
        ...
        #                    print(np.mean([np.mean(x) for x in hard_predictions]))
                            ...
                            pairwise_4classifier = pairwise_classifiers._pairwise_4classifier(base_learner, n_labels, 
                                                                                    X[train_index], Y[train_index])
                            predicted_Y, predicted_ranks = predictors._preorders(n_labels, X[test_index], Y[test_index],
                                              known_indices, pairwise_4classifier, calibrated_2classifier)                            
                            hamming_loss_pairwise_4classifier = predictors._hamming(base_learner, predicted_Y, Y[test_index], known_indices)
                            ...
                            f1_pairwise_4classifier = predictors._f1(base_learner, predicted_Y, Y[test_index], known_indices)
                            average_f1_pairwise_4classifier.append(f1_pairwise_4classifier)
                            print(f1_pairwise_4classifier)
                            jaccard_pairwise_4classifier = predictors._jaccard(base_learner, predicted_Y, Y[test_index], known_indices)
                            average_jaccard_pairwise_4classifier.append(jaccard_pairwise_4classifier)
                            print(jaccard_pairwise_4classifier)
                            subset0_1_pairwise_4classifier = predictors._subset0_1(base_learner, predicted_Y, Y[test_index], known_indices)
                            average_subset0_1_pairwise_4classifier.append(subset0_1_pairwise_4classifier)
                            print(subset0_1_pairwise_4classifier)
                            recall_pairwise_4classifier = predictors._recall(base_learner, predicted_Y, Y[test_index], known_indices)
        ...
        #                    predicted_Y = ECC.predict(X[test_index].astype(float))
                            print("====================== ECC ======================")
                            hamming_loss_ECC = predictors._hamming(base_learner, predicted_Y, Y[test_index], known_indices)
                            average_hamming_loss_ECC.append(hamming_loss_ECC)
                            print(hamming_loss_ECC)
                            f1_ECC = predictors._f1(base_learner, predicted_Y, Y[test_index], known_indices)
                            average_f1_ECC.append(f1_ECC)
                            print(f1_ECC)
                            jaccard_ECC = predictors._jaccard(base_learner, predicted_Y, Y[test_index], known_indices)
                            average_jaccard_ECC.append(jaccard_ECC)
                            print(jaccard_ECC)
                            subset0_1_ECC = predictors._subset0_1(base_learner, predicted_Y, Y[test_index], known_indices)
                            average_subset0_1_ECC.append(subset0_1_ECC)
                            print(subset0_1_ECC)
                            recall_ECC = predictors._recall(base_learner, predicted_Y, Y[test_index], known_indices)
                    ...
                    res_file = "addition_partial_noisy_4_w_try_compareAcc_%i_%i_%i_%i_%s_%s" %(int(noisy_rate*10), int(partial_prob*10),total_repeat, folds, dataFile, base_learner)
