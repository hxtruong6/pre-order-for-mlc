...
def make_table(all_rows, meta):
    ...
    for solver, height, _color, _mark, legend in CURVES:
        ...
        for order, metric in TABLE_COLS:
            ...
            # GLPK pre-order times are huge (tens of seconds to minutes); show
            # them in seconds so the cell stays readable, with an explicit unit.
            to_sec = solver == "glpk" and order == "preorder"
            if to_sec:
                v /= 1000.0
            if extrapolated:                      # avoid false precision
                d = int(np.floor(np.log10(v)))
                v = round(v, -(d - 1))            # 2 significant figures
            txt = (f"{v:.0f}\\,s" if to_sec else fmt_ms(v))
            cells.append(txt + ("\\textsuperscript{*}" if extrapolated else ""))
    ...
    full = (
        ...
        "(enron), in milliseconds unless marked s (seconds). The four ILP "
        "rows are the BOPOs search times plotted in the runtime figures; the "
        "lower block lists the baseline per-instance inference costs "
        "(independent of preference order and target metric). GLPK is "
        "measured only up to $K\\le31$ (prohibitively slow beyond), so its "
        "$K=53$ entries (\\textsuperscript{*}) are extrapolated from the "
        "least-squares power-law fit $c\\,K^{x}$; HiGHS values are measured.}\n"
