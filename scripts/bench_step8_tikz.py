...
def make_table(all_rows, meta):
    ...
    dnf_note = (
        " GLPK does not complete the pre-order search at $K=53$: the solve is "
        "terminated after several minutes without returning a solution "
        "(exhausting memory), so those entries are marked "
        "\\textsuperscript{$\\dagger$}n/a. This is precisely why the "
        "full-size enron experiments use HiGHS." if any_dnf else "")
    full = (
