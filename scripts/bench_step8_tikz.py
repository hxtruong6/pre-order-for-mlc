...
def load_glpk_k53_measured():
    ...
    (order, metric, height), from bench_step9's raw log if present."""
    path = SCRATCH / "glpk_k53_raw.csv"
    if not path.exists():
        return {}
    agg = {}
    with open(path) as f:
        for r in csv.DictReader(f):
            agg.setdefault((r["order"], r["metric"], r["height"]), []).append(
                float(r["solve_s"]))
...
def make_table(all_rows, meta):
    ...
    for solver, height, _color, _mark, legend in CURVES:
        ...
        for order, metric in TABLE_COLS:
            ...
            if solver == "glpk" and key in measured:
                v, extrapolated = measured[key][0], False   # direct measurement
            else:
                v, extrapolated = value_at_53(all_rows[order], solver, height,
                                              metric)
    ...
    extrap_note = (
        " Entries marked \\textsuperscript{*} are extrapolated from the "
        "least-squares power-law fit $c\\,K^{x}$ (GLPK is prohibitively slow "
        "to measure at those sizes)." if any_extrapolated else "")
    full = (
        ...
        f"order and target metric). GLPK values are averaged over {nspec} "
        "test instances measured directly at $K=53$; HiGHS and baseline "
        f"values are measured over the full test set.{extrap_note}}}\n"
