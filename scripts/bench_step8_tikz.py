...
def make_table(all_rows, meta):
    ...
    dnf_note = (
        " GLPK is impractically slow for the pre-order search at $K=53$: a "
        "single instance did not finish within 15 minutes of solving "
        "(vs.\\ under a second for HiGHS), so those entries are left "
        "\\textsuperscript{$\\dagger$}n/a rather than timed. This is "
        "precisely why the full-size enron experiments use HiGHS." if any_dnf
        else "")
    full = (
