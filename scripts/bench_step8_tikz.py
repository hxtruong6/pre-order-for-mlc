...
def load_glpk_k53_measured():
    """Directly measured GLPK per-instance times at K=53 (ms), keyed by
    (order, metric, height), pooled from bench_step9 (partial) and
    bench_step10 (pre-order) raw logs if present."""
    agg = {}
    paths = [SCRATCH / "glpk_k53_raw.csv",
             SCRATCH / "glpk_k53_preorder_raw.csv"]
    paths += sorted(SCRATCH.glob("glpk_k53_preorder_p_*.csv"))  # parallel runs
    for path in paths:
        if not path.exists():
            continue
        with open(path) as f:
            for r in csv.DictReader(f):
                agg.setdefault((r["order"], r["metric"], r["height"]),
                               []).append(float(r["solve_s"]))
    return {k: (np.mean(v) * 1e3, len(v)) for k, v in agg.items()}
...
def make_table(all_rows, meta):
    ...
    B("\\midrule")
    measured = load_glpk_k53_measured()       # (order,metric,height) -> (ms, n)
    n_meas = sorted({n for _v, n in measured.values()})
    any_dnf = False
    for solver, height, _color, _mark, legend in CURVES:
        cells = []
        for order, metric in TABLE_COLS:
            key = (order, metric, height)
            if solver == "glpk":
                if key in measured:               # timed directly at K=53
                    cells.append(fmt_ms(measured[key][0]))
                else:                             # GLPK cannot finish this case
                    any_dnf = True
                    cells.append("n/a\\textsuperscript{$\\dagger$}")
                continue
            v, _ = value_at_53(all_rows[order], solver, height, metric)  # HiGHS
    ...
    nspec = (f"{n_meas[0]}" if len(n_meas) == 1 else
             f"{min(n_meas)}--{max(n_meas)}") if n_meas else "10"
    dnf_note = (
        " For the pre-order search GLPK is impractically slow at full "
        "transitivity: a single $K=53$ instance did not finish after more "
        "than four hours of solving (vs.\\ under a second for HiGHS), so "
        "those two entries are left \\textsuperscript{$\\dagger$}n/a. The "
        "height-2 restriction is tractable but still about three orders of "
        "magnitude slower than HiGHS. This is precisely why the full-size "
        "enron experiments use HiGHS." if any_dnf
        else "")
    full = (
        ...
        f"order and target metric). Measured GLPK entries are averaged over "
        f"{nspec} test instances timed directly at $K=53$; HiGHS and baseline "
