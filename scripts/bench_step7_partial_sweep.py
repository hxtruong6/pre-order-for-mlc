"""Step 7 of the runtime-scaling benchmark (reviewer 1 response).

PARTIAL_ORDER counterpart of bench_step2_full. Times the average per-instance
partial-order ILP solve on the real enron 3-class pairwise probabilities as the
label-set size K grows, for both solvers and both heights. GLPK is capped at
K<=31. Writes scaling_results_partial.csv.
"""
...
from preorder4mlc.searching_algorithms import Search_BOParOs  # noqa: E402
...
def time_config(proba_sub, L, n_test, metric, height, n_timed):
    search = Search_BOParOs(proba_sub, L, n_test, metric, height=height)
    iv, ind = {}, 0
    for i in range(L - 1):
        for j in range(i + 1, L):
            for l in range(3):            # partial order: 3 relations per pair
                iv[f"{i}_{j}_{l}"] = ind
                ind += 1
    t = time.perf_counter()
    ...
    for n in range(min(n_timed, n_test)):
        t = time.perf_counter()
        search._solve_one_PARTIAL_ORDER(n, ii, jj, iv, G, h, A, b, I, B)
        st.append(time.perf_counter() - t)
    return enc, np.array(st)


def main():
    proba_full = np.load(SCRATCH / "enron_proba_partial.npy")
    n_test = proba_full.shape[2]
    print(f"loaded proba {proba_full.shape}", flush=True)

    out = SCRATCH / "scaling_results_partial.csv"
            ...
            for L in grid:
                ...
                for metric in METRICS:
                    for height in HEIGHTS:
                        ...
                        w.writerow(row)
                        f.flush()
                        print(f"[{solver}] K={L:2d} {metric.name:7s} "
                              f"{row['height']:4s} n={len(st):3d} "
                              f"mean={row['mean_solve_s']*1e3:8.2f}ms",
                              flush=True)
    print("DONE -> scaling_results_partial.csv", flush=True)
