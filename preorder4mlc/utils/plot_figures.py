...
def pivot_metric(
    ...
    sub = long_df[
        (long_df["ptype"] == ptype) & (long_df["metric"] == metric) & (long_df["noise"] == noise)
    ]
    if sub.empty:
        return pd.DataFrame()
...
def friedman_p(sig_df: pd.DataFrame, ptype: str, metric: str, noise: str) -> float | None:
    if sig_df.empty:
        return None
    row = sig_df[
        (sig_df["ptype"] == ptype) & (sig_df["metric"] == metric) & (sig_df["noise"] == str(noise))
    ]
    if row.empty:
        return None
...
def plot_cd_diagram(
    ...
    for idx, (rank, name_i) in enumerate(zip(sorted_ranks, sorted_names)):
        if idx < half:
            ...
            ax.text(lo - 0.4, y, f"{name_i}  ({rank:.2f})", ha="right", va="center", fontsize=9)
        else:
            y = -0.5 * (k - idx) - 0.3
            ax.plot([rank, rank], [y_axis, y], color="black", lw=0.8)
            ax.plot([rank, hi + 0.3], [y, y], color="black", lw=0.8)
            ax.text(hi + 0.4, y, f"({rank:.2f})  {name_i}", ha="left", va="center", fontsize=9)
    ...
    for ptype, metric in CD_TARGETS:
        for noise in NOISE_LEVELS:
            ...
            if n_datasets < 2 or k < 3:
                print(
                    f"[warn] insufficient data for CD {ptype}/{metric}/{noise}: "
                    f"n={n_datasets}, k={k}"
                )
                continue
...
def make_noise_curves(results_dir: str, out_dir: str) -> None:
    ...
    for ptype, metric, pretty in targets:
        ...
        ax.legend(ncol=2, loc="upper left", bbox_to_anchor=(1.01, 1.0), frameon=False, fontsize=8)
        fig.tight_layout()
...
def _scatter_two(
    ...
    merged = xs.merge(
        ys,
        on=["dataset", "ptype", "algorithm", "noise"],
        suffixes=("_x", "_y"),
    )
    ...
    noise_handles = [
        Line2D(
            ...
            marker="o",
            linestyle="None",
            markerfacecolor=noise_palette[n],
            markeredgecolor="black",
            markersize=9,
            label=f"noise={n}",
        )
...
def make_tradeoff_plot(long_df: pd.DataFrame, out_dir: str) -> None:
    ...
    ax.grid(True, ls=":", alpha=0.4)
    ax.legend(ncol=2, loc="upper left", bbox_to_anchor=(1.01, 1.0), fontsize=8, frameon=False)
    fig.tight_layout()
...
def make_best_variant_heatmap(long_df: pd.DataFrame, out_dir: str) -> None:
    bv = long_df[(long_df["ptype"] == "BinaryVector") & (long_df["metric"] == "hamming_accuracy")]
    if bv.empty:
...
def main() -> None:
    ...
    print(
        f"[info] long DataFrame: {len(long_df):,} rows; "
