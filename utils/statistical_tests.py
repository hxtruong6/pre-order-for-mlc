...
def load_long(summaries: list[tuple[str, str, str]]) -> pd.DataFrame:
    ...
    return pd.DataFrame(rows, columns=["dataset", "ptype", "algorithm", "metric", "noise", "mean"])
...
def plot_cd_diagram(
    ...
    for idx, (rank, name) in enumerate(zip(sorted_ranks, sorted_names)):
        if idx < half:
            ...
            ax.text(lo - 0.4, y, f"{name}  ({rank:.2f})", ha="right", va="center", fontsize=9)
        else:
            y = -0.5 * (k - idx) - 0.3
            ax.plot([rank, rank], [y_axis, y], color="black", lw=0.8)
            ax.plot([rank, hi + 0.3], [y, y], color="black", lw=0.8)
            ax.text(hi + 0.4, y, f"({rank:.2f})  {name}", ha="left", va="center", fontsize=9)
...
def run(results_dir: str, output_dir: str, alpha: float = 0.05) -> None:
    ...
    for (ptype, metric, noise), grp in long_df.groupby(["ptype", "metric", "noise"]):
        pivot = grp.pivot_table(index="dataset", columns="algorithm", values="mean", aggfunc="mean")
        ...
        try:
            chi2, p_val = friedmanchisquare(*[complete[col].values for col in complete.columns])
        except ValueError:
        ...
        # Average ranks CSV
        rank_df = pd.DataFrame({"algorithm": algs, "avg_rank": avg_ranks}).sort_values("avg_rank")
        rank_path = os.path.join(output_dir, f"avg_ranks_{ptype}_{metric}_{noise}.csv")
        rank_df.to_csv(rank_path, index=False)

        # Nemenyi pairwise
        pvals = nemenyi_pvalues(ranks, n_datasets=n_datasets)
        nemenyi_df = pd.DataFrame(pvals, index=algs, columns=algs)
        nemenyi_path = os.path.join(output_dir, f"nemenyi_{ptype}_{metric}_{noise}.csv")
    ...
    summary_df = pd.DataFrame(summary_rows).sort_values(["ptype", "metric", "noise"])
...
def main() -> None:
    ...
    ap.add_argument("--output_dir", default="results/final_20260514_v2_summary/stats")
