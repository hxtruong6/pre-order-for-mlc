...
#        for noisy_rate in [0.2, 0.4]:
    ...
    #                    print(np.mean([np.mean(x) for x in hard_predictions]))
                        ...
                        pairwise_3classifier = pairwise_classifiers._pairwise_3classifier(base_learner, n_labels, 
                                                                                X[train_index], Y[train_index])
                        predicted_Y, predicted_ranks = predictors._partialorders(n_labels, X[test_index], pairwise_3classifier, calibrated_2classifier)
    ...
    #                    print(np.mean([np.mean(x) for x in hard_predictions]))
                        ...
                        pairwise_4classifier = pairwise_classifiers._pairwise_4classifier(base_learner, n_labels, 
                                                                                X[train_index], Y[train_index])
                        predicted_Y, predicted_ranks = predictors._preorders(n_labels, X[test_index], pairwise_4classifier, calibrated_2classifier)
            #            print(hard_predictions)
    ...
    #                    predicted_Y = ECC.predict(X[test_index].astype(float))
                ...
                res_file = "noisy_4_w_try_compareAcc_%i_%i_%i_%s_%s" %(int(noisy_rate*10), total_repeat, folds, dataFile, base_learner)
